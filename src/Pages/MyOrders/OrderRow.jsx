
const OrderRow = ({ order,handleDeleteOrder ,handleOrderConfirm}) => {
  const {_id, img, service, date, price, status} = order;
  
  return (

    <tr>
      <th>
          <button onClick={()=>handleDeleteOrder(_id)} className="btn btn-circle btn-sm">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
      </th>
      <td>
        <div className="flex items-center gap-3">
          <div className="avatar">
            <div className=" rounded-lg w-24 h-24">
              <img src={img} />
            </div>
          </div>
        </div>
      </td>
      <td>
        <p> {service}</p>
      </td>
      <td>{date}</td>
      <td>{price}</td>
      <th>
       {
        status === 'confirm' ? <span className="font-bold text-blue-700">Confirmed</span> : <button onClick={()=>handleOrderConfirm(_id)} className="btn btn-ghost btn-xs">Please Confirm</button>
 
       }
      </th>
    </tr>
  );
};

export default OrderRow;